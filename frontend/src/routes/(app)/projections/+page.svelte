...
<div class="p-8 space-y-6">
  <div>
    ...
    <p class="text-brand-surface-2 mt-2">Visualize your financial commitments and liquidity forecast</p>
  </div>
  ...
  {:else if projections}
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="card">
        <div class="text-sm text-brand-surface-2 mb-2 uppercase tracking-wider">Total Commitments</div>
      ...
      <div class="card">
        <div class="text-sm text-brand-surface-2 mb-2 uppercase tracking-wider">Pending</div>
      ...
      <div class="card">
        <div class="text-sm text-brand-surface-2 mb-2 uppercase tracking-wider">Overdue</div>
        <div class="text-3xl font-bold text-brand-rose">
          ...
          <tbody class="divide-y divide-brand-surface-2">
            {#each projections.occurrences.slice(0, 20) as occurrence (occurrence.id)}
              <tr class="hover:bg-brand-surface-2 transition-colors">
                ...
                <td class="py-3 px-3 text-brand-surface-2">{occurrence.entity_name || '-'}</td>
                <td class="py-3 px-3 text-right font-semibold text-brand-rose">
                  {formatCurrency(occurrence.amount)}
                </td>
                <td class="py-3 px-3">{formatDateDisplay(occurrence.due_date)}</td>
                <td class="py-3 px-3">
                  <span
                    class="px-2 py-1 rounded text-xs font-semibold
                      {occurrence.status === 'processed' ? 'bg-brand-emerald/20 text-brand-emerald' : ''}
                      {occurrence.status === 'pending' ? 'bg-brand-cyan/20 text-brand-cyan' : ''}
                      {occurrence.status === 'overdue' ? 'bg-brand-rose/20 text-brand-rose' : ''}"
                  >
                    {occurrence.status}
                  </span>
                </td>
